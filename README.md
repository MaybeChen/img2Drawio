...
3. **VLM element recognition**: identifies non-text and non-connector visual elements through an OpenAI-compatible custom service. Runtime settings are loaded from `config.yaml`, including `base_url`, `model`, `x_hw_id`, and `x_hw_appkey`. Requests include `X-HW-ID` and `X-HW-APPKEY` headers, and OCR/YOLO boxes are passed as optional exclusion context.
